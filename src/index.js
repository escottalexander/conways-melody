				...
				for (let j = 0; j < this.cols; j++) {
					...
					if (!g[i][j] && count === 3) {
						g2[i][j] = true;
							...
							this.toneGen("sawtooth",293.7	, 1)
						} else if ((Math.floor(Math.random() * 80)) === 2) {
							this.toneGen("sawtooth",370.0, 1)
						} else if ((Math.floor(Math.random() * 80)) === 3) {
							this.toneGen("sawtooth",440.0, 1)
						} 
					}
				}
			}
